...
## select top genera and DMM alpha coef from AGP
```{r}
...
save(df_gutzilla, file="../df_gutzilla.rda")

```



## combine CMD and AGP DMM weights

```{r fig.height=5, fig.width=10}

load("../df_gutzilla.rda")
...
merge(df_curated_v3 %>% select(-m19,-m22,-m23,-m24),df_gutzilla, by="row.names") %>% 
  ...
  mutate(dataset=ifelse(Microbiota %>% grepl("\\M", .), "AGP","CMD")) %>%
...
save(microbiota_annotation, file="figures/microbiota_annotation.rda")
...
save(microbiota_types, file="figures/microbiota_types.rda")
...
ann_colors = list(
    dataset = c(AGP="pink", CMD="lightblue"),
  ...
  #as.matrix %>% #log10() %>%
  pheatmap::pheatmap(mat=log10(microbiota_types), 
                     cutree_cols = 7, 
                     annotation_col = microbiota_annotation, 
                     cellwidth = 10, cellheight = 20, 
                     clustering_distance_cols = microbiota_types_jsd, 
                     annotation_colors = ann_colors, 
                     color=viridis::viridis_pal()(100),
                     labels_row = as.expression(lapply(rownames(microbiota_types),
                                         function(x) bquote(italic(.(x)))))
                                         ) %>% ggplotify::as.ggplot() -> fig3A

save(fig3A, file="figures/fig3A.rda")
