...
load("agp_metadata_enterotypes_num.rda")
...
metadata=read.csv2(system.file("data-raw/Metadata_10317_20191022-112414_curatedv4_VSv1.csv", package = "gutzilla"), stringsAsFactors = FALSE) %>% mutate(across(where(is.character), ~na_if(., "")))
...
metadata %>%
  ...
  mutate(branches = case_when(Enterotypes_id %in% c(18,14,16,17,7) ~ "Prevotella DMM partitions",
                              Enterotypes_id %in% c(11,15,6,8,10,3,4,2) ~ "Bacteroides DMM partitions",
                              Enterotypes_id %in% c(12,1,5,9) ~ "Clostridiales DMM partitions",
                              Enterotypes_id %in% c(13) ~ "Akkermansia DMM partitions",
                              TRUE ~ "outliers")
         ) %>%
  ...
  facet_wrap(~branches_before) + xlab("time range between two samples") + ylab("Samples %") + scale_x_discrete(labels=c("< 1 week","1 week to 4 months", "4 to 12 months", "> 12 months")) -> figS9A

figS9A


ggsave("figures/FigS9A_branche_before_after.pdf")
...
enterotypes_time_series_network_random %>%
  mutate(branches = case_when(Enterotypes_id %in% c(18,14,16,17,7) ~ "*Prevotella*<br>DMM partitions",
                              Enterotypes_id %in% c(11,15,6,8,10,3,4,2) ~ "*Bacteroides*<br>DMM partitions",
                              Enterotypes_id %in% c(12,1,5,9) ~ "*Clostridiales*<br>DMM partitions",
                              Enterotypes_id %in% c(13) ~ "Akk.",
                              TRUE ~ "outliers")
         ) %>%
  mutate(branches_before = case_when(Enterotypes_id_before %in% c(18,14,16,17,7) ~ "*Prevotella*<br>DMM partitions",
                              Enterotypes_id_before %in% c(11,15,6,8,10,3,4,2) ~ "*Bacteroides*<br>DMM partitions",
                              Enterotypes_id_before %in% c(12,1,5,9) ~ "*Clostridiales*<br>DMM partitions",
...
ggplot() + geom_bar(aes(x=paste0("M",Enterotypes_id_before),y=p,fill=dataset),stat="identity", position = "dodge") +
  ...
  theme(legend.position="bottom", strip.text = ggtext::element_markdown()) -> fig5M_stability
...
ggraph(graph, weights=p, layout="stress") +
  ...
  scale_fill_brewer("DMM partitions", type="qual") +
  ...
  theme_void() + theme(legend.text = element_text(face="italic")) -> fig5A
...
ggraph(graph, weights=p, layout="stress") +
  ...
  #geom_node_point(aes(color=branches,size=stability*2)) +
  #geom_node_point(aes(color=branches,size=stability)) +
  geom_node_label(aes(label=paste0(partition_names,branches %>% stringr::str_sub(start=1, end=1)), 
                      fill=paste0(branches %>% gsub("DMM types","",.)," (",branches %>% stringr::str_sub(start=1, end=1),")"), size=stability)) + 
  #geom_node_text(aes(label=partition_names), repel=TRUE) +
  #scale_color_grey() +
  scale_fill_grey("main taxa DMM partitions") +
  scale_alpha_continuous("participants proportion") + 
  scale_edge_alpha("ind. proportion (switch)") + 
  scale_edge_color_continuous("ind. proportion (switch)", low = "grey", high ="black", guide = FALSE) +
  scale_size_continuous("ind. proportion (stable)", trans="log10", limits = c(0.1,0.9), breaks = c(0.3,0.6,0.9)) +
  theme_void()
...
f_b_ratio %>%
  ...
  #geom_vline(xintercept = 1/5) + 
  cowplot::theme_cowplot() +
  ylab("Bacteroides DMM partitions") +
  xlab("Faecalibacterium Bacteroides ratio") -> p_fprau_partition_bw


p_fprau_density/p_fprau_partition_bw
...
f_b_ratio %>%
  ...
  filter(branches=="Bacteroides DMM types") %>%
  merge(shannon, by.x="sample_name", by.y="row.names") %>%
  mutate(substates = "High (m2, m4, m8)") %>%
  mutate(substates = ifelse(Enterotypes_id %in% c("11","6","10","3"), "Medium (m3, m6, m10, m11)",substates)) %>%
  mutate(substates = ifelse(Enterotypes_id %in% c("15"), "Low (m15)",substates)) %>%
  mutate(substates = substates %>% forcats::fct_relevel("High (m2, m4, m8)",after=2 )) %>%
  ggplot() + 
  geom_point(aes(x=ratio,y=shannon, col=substates), alpha=0.5)  + 
  scale_x_continuous("Faecalibacterium:Bacteroides ratio\n from samples classified in\nBacteroides enriched branch",trans="log2", labels=scales::trans_format("frac_fun",scales::label_math(expr = 1/.x, format = force))) +
  geom_vline(xintercept = 1/4, col="red") +
  geom_vline(xintercept = 1/64, col="red") +
  scale_color_brewer("Microbiota DMM Partitions\nclassified thank to\nFaecalibacterium:Bacteroides ratio", palette = 8, type="div") +
  theme_classic()
...
f_b_ratio %>%
  ...
  mutate(substates = substates %>% forcats::fct_relevel("High (m2, m4, m8)",after=2 )) %>%
  ggplot() + 
  geom_point(aes(x=ratio,y=shannon, col=substates), alpha=0.7)  + 
  scale_x_continuous("Faecalibacterium:Bacteroides ratio\n from samples classified in\nBacteroides enriched branch",trans="log2", labels=scales::trans_format("frac_fun",scales::label_math(expr = 1/.x, format = force))) +
  geom_vline(xintercept = 1/4, col="black", linetype=2, size=1.5) +
  geom_vline(xintercept = 1/64, col="black", linetype=2, size=1.5) +
  scale_colour_grey("Microbiota DMM Partitions\nclassified thank to\nFaecalibacterium:Bacteroides ratio") +
  #scale_color_brewer("Microbiota DMM Partitions\nclassified thank to\nFaecalibacterium:Bacteroides ratio", palette = 8, type="div") +
  theme_classic()
...
y_lab_fig5BCinset = ylab("*Faecalibacterium*/<br>*Bacteroides*<br>ratio variation")

x_lab_fig5BC = xlab("*Faecalibacterium* /<br>*Bacteroides*<br>ratio") 

x_lab_fprau = xlab("*Faecalibacterium* / *Bacteroides* ratio<br>within *Bacteroides* enriched branch") 

y_lab_fprau = ylab("*Bacteroides*<br>DMM partitions")

theme_md = theme(axis.title.x = ggtext::element_markdown(), axis.title.y = ggtext::element_markdown())
...
(
  (
    ...
    (
      (
        (
          (fig5Binset+ylab("*Faecalibacterium* /<br>*Bacteroides*<br>ratio variation")+theme_md)/
            (fig5Cinset+ylab("*Faecalibacterium* /<br>*Bacteroides*<br>ratio variation")+theme_md) |
            (fig5B_full_scaled + x_lab_fig5BC + theme_md)/
            (fig5C_full_scaled + x_lab_fig5BC + theme_md))/
          (
            (p_fprau_density+geom_vline(xintercept = 1/4) + x_lab_fprau + theme_md)/
              (p_fprau_partition+  geom_vline(xintercept = 1/4) + x_lab_fprau + y_lab_fprau + theme_md) + 
...
ggsave(file="figures/figure3_network_time_series.pdf", height=12, width=16)

save(fig5Binset, file="figures/fig3c.rda")
save(fig5Cinset, file="figures/fig3d.rda")
save(p_fprau_partition, file="figures/fig3h.rda")
```
...
genus %>% tibble::column_to_rownames("taxa") %>% phyloseq::otu_table(taxa_are_rows=TRUE) -> genus_for_phyloseq

genus %>%
  select(taxa) %>%
  tidyr::separate(taxa, into=c("k","p","c","o","f","g"),remove = FALSE, sep = ";") %>%
  tibble::column_to_rownames("taxa") %>% as.matrix %>% phyloseq::tax_table() -> taxa_for_phyloseq


pseq=phyloseq::phyloseq(OTU=genus_for_phyloseq, phyloseq::sample_data(metadata_for_physeq), TAX=taxa_for_phyloseq )
