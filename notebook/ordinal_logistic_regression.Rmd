...
save(metadata_enterotypes_num, file="agp_metadata_enterotypes_num.rda")
...
load("regm_tidy.rda")  
  ...
  regm_tidy %>%
    ...
    merge(enterotypes_shannon_median, by.x="y.level", by.y="Enterotypes_id") %>%
  mutate(branch ="Clostridiales DMM partitions") %>%
  mutate(branch = ifelse(y.level %in% c("m18","m14","m16","m17","m7"), "Prevotella DMM partitions",branch)) %>%
  mutate(branch = ifelse(y.level %in% c("m11","m15","m6","m8","m10","m3","m4","m2"), "Bacteroides DMM partitions",branch)) %>%
...
p_cut_off=0.10
  ...
  regm_tidy %>%
  group_by(variable) %>% mutate(p.value = p.adjust(p.value, method="fdr")) %>%
    mutate(variable_group = "disease") %>%
   mutate(variable_group = 
  ...
  mutate(branch = ifelse(y.level %in% c("m13"), "Akk.",branch)) %>%
  mutate(estimate = log(estimate)) %>%
  mutate(estimate2 = ifelse(p.value>p_cut_off,0,estimate)) %>%
  group_by(variable) %>%
  mutate(s=mean(abs(estimate2))) %>% 
  filter(s>0) %>%
  ungroup() %>%
  ...
  mutate(variable = variable %>% as.character %>% as.factor() %>% forcats::fct_reorder(s, .desc = TRUE)) %>% 
     mutate(branch = branch %>% gsub("DMM partitions", "<BR>DMM partitions",.)) %>%
  ggplot() + 
    geom_tile(aes(y=y.level, x=variable, fill=estimate)) + 
    geom_point(aes(y=y.level, x=variable, alpha=p.value<p_cut_off)) + 
    scale_fill_gradient2("Log OR", low=scales::muted("blue"), mid="white", high=scales::muted("red")) +
    facet_grid(branch~variable_group, scale="free", space = "free") + 
    cowplot::theme_cowplot() +
    scale_alpha_discrete("FDR < 10%") +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1), strip.text.y = ggtext::element_markdown()) +
...
regm_tidy %>%
  ...
  cowplot::theme_cowplot() +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(), legend.text = ggtext::element_markdown()) -> fig4B
 ...
 regm_tidy %>%
  ...
  facet_grid(branch~., scale="free", space = "free") + scale_fill_brewer("",type="qual", palette = "Set2") +
  ylab("Microbiome partition\nordered by branch and by alpha-diversity") + xlab("absoluted cummulated model weights") + 
   cowplot::theme_cowplot() +
   theme(strip.text.y = ggtext::element_markdown())  -> fig4C

fig4A = fig4A + ylab("")

 ((fig4D + fig4B + plot_layout(widths = c(1,3))) /
   (fig4C + fig4A + plot_layout(widths = c(1,3.5))) + plot_layout(heights = c(1,3)) )+ plot_annotation(tag_levels = "a")
 
 ggsave("figures/figure4.pdf", h=15, w=26)
 

```


## focus on Bacteroides branches
...
```{r fig.height=14, fig.width=10}
  ...
  group_by(variable) %>% mutate(p.value = p.adjust(p.value, method="fdr")) %>%
   ...
   mutate(variable_group = 
             case_when(variable %in% c("AGE_YEARS","SEX","BMI","BOWEL_MOVEMENT_FREQUENCY") ~ "Intrinsic\nHost",
                       ...
                       grepl("ALCOHOL", variable) ~ "Diet",
                       variable %in% c("DIET_TYPE","TYPES_OF_PLANTS") ~ "Diet",
                       grepl("SPECIALIZED", variable) ~"Spec.\nDiet",
                       grepl("REGION", variable) ~ "Reg. of\nBirth",
                       TRUE ~ "Medical history"
                            )) %>%
    merge(enterotypes_shannon_median, by.x="y.level", by.y="Enterotypes_id") %>%
  mutate(branch ="Clostridiales DMM partitions") %>%
  ...
  mutate(branch = ifelse(y.level %in% c("m11","m15","m6","m8","m10","m3","m4","m2"), "Bacteroides DMM partitions",branch)) %>%
  mutate(branch = ifelse(y.level %in% c("m13"), "Akk.",branch)) %>%
  mutate(substates = "High") %>%
  mutate(substates = ifelse(y.level %in% c("m11","m6","m10","m3"), "Medium",substates)) %>%
  mutate(substates = ifelse(y.level %in% c("m15"), "Low",substates)) %>%
  filter(branch == "Bacteroides DMM partitions") %>%
  mutate(substates = substates %>% forcats::fct_relevel("High",after=2 )) %>%
  mutate(estimate = log(estimate)) %>%
  mutate(estimate2 = ifelse(p.value>p_cut_off,0,estimate)) %>%
  group_by(variable) %>%
  mutate(s=mean(abs(estimate2))) %>% 
  ...
  mutate(variable = gsub("_|\\."," ", variable)) %>%
  mutate(variable = variable %>% as.character %>% as.factor() %>% forcats::fct_reorder(s, .desc = TRUE)) %>% 
     mutate(branch = branch %>% gsub("DMM partitions", "\nDMM partitions",.)) %>%
  ggplot() + 
    geom_tile(aes(y=y.level, x=variable, fill=estimate)) + 
    geom_point(aes(y=y.level, x=variable, alpha=p.value<p_cut_off)) + 
    #scale_fill_gradient2("Log OR", low=scales::muted("blue"), mid="white", high=scales::muted("red")) +
    scale_fill_gradient("Log OR", low="white", high="#1a1a1a") +  
    facet_grid(variable_group~substates, scale="free", space = "free") + 
    cowplot::theme_cowplot() +
    scale_alpha_discrete("FDR < 10%") +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
    ylab("Microbiome Bacteroides partitions") + xlab("AGP questionnaire data") + coord_flip() 
...
metadata_enterotypes_num %>%
  mutate(branch ="Clostridiales DMM types") %>%
  mutate(branch = ifelse(Enterotypes_id %in% c("m18","m14","m16","m17","m7"), "Prevotella DMM types",branch)) %>%
  mutate(branch = ifelse(Enterotypes_id %in% c("m11","m15","m6","m8","m10","m3","m4","m2"), "Bacteroides DMM types",branch)) %>%
  mutate(branch = ifelse(Enterotypes_id %in% c("m13"), "Akkermansia DMM types",branch)) %>%
  filter(branch=="Bacteroides DMM types" | Enterotypes_id =="m5") %>%
  select(SALTED_SNACKS_FREQUENCY,VEGETABLE_FREQUENCY,SUGARY_SWEETS_FREQUENCY,TYPES_OF_PLANTS,FRUIT_FREQUENCY,ANTIBIOTIC_HISTORY) %>%
  mutate_all(as.numeric) %>%
  ade4::dudi.pca(scannf=FALSE, nf=2) -> metadata_enterotypes_pca_branch
  


metadata_enterotypes_num_shannon = merge(metadata_enterotypes_num,shannon, by="row.names")
  

metadata_enterotypes_num_shannon %>%
  mutate(branch ="Clostridiales DMM types") %>%
  mutate(branch = ifelse(Enterotypes_id %in% c("m18","m14","m16","m17","m7"), "Prevotella DMM types",branch)) %>%
  mutate(branch = ifelse(Enterotypes_id %in% c("m11","m15","m6","m8","m10","m3","m4","m2"), "Bacteroides DMM types",branch)) %>%
  mutate(branch = ifelse(Enterotypes_id %in% c("m13"), "Akkermansia DMM types",branch)) %>%
  filter(branch=="Bacteroides DMM types" | Enterotypes_id =="m5") %>%
  select(TYPES_OF_PLANTS, shannon, Enterotypes_id) %>%
  mutate(Enterotypes_id= forcats::fct_reorder(Enterotypes_id, shannon)) %>%
  #mutate(TYPES_OF_PLANTS=TYPES_OF_PLANTS%>% as.numeric()) %>%
  ggplot() + 
  geom_histogram(aes(x=TYPES_OF_PLANTS,fill=Enterotypes_id), stat="count", position="fill")


 merge(metadata_enterotypes_num_shannon,metadata_enterotypes_pca_branch$li, by.x="Row.names", by.y="row.names") %>% 
   mutate(branch ="Clostridiales DMM types") %>%
  mutate(branch = ifelse(Enterotypes_id %in% c("m18","m14","m16","m17","m7"), "Prevotella DMM types",branch)) %>%
  mutate(branch = ifelse(Enterotypes_id %in% c("m11","m15","m6","m8","m10","m3","m4","m2"), "Bacteroides DMM types",branch)) %>%
  mutate(branch = ifelse(Enterotypes_id %in% c("m13"), "Akkermansia DMM types",branch)) %>%
  filter(branch=="Bacteroides DMM types" | Enterotypes_id =="m5") %>%
  mutate(Enterotypes_id= forcats::fct_reorder(Enterotypes_id, shannon)) %>%
   select(Axis1,shannon,Enterotypes_id) %>%
   ggplot() + 
  geom_point(aes(x=Axis1,y=shannon,col=Enterotypes_id), size=1,alpha=0.5)
...
merge(metadata_enterotypes_num_shannon,metadata_enterotypes_pca_branch$li, by.x="Row.names", by.y="row.names") %>% 
  ...
  mutate(Enterotypes_id= forcats::fct_reorder(Enterotypes_id, shannon)) %>%
   select(Axis1,shannon,Enterotypes_id) %>%
  ggplot() + 
  geom_histogram(aes(x=rank((-Axis1))+1,fill=Enterotypes_id), position="fill", bins=30) +
  scale_fill_viridis_d()  + scale_x_log10()
...
merge(metadata_enterotypes_num_shannon,metadata_enterotypes_pca_branch$li, by.x="Row.names", by.y="row.names") %>% 
  ...
  mutate(Enterotypes_id= forcats::fct_reorder(Enterotypes_id, shannon)) %>%
   select(Axis1,shannon,Enterotypes_id) %>%
  ggplot() + 
  geom_point(aes(x=rank((-Axis1))+1,y=shannon)) +
